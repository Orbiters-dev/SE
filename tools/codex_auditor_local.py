...
【필수 규칙】
- 4분야 = 총 20개 확인 (tips:8, brand:6, k_babyfood:3, knowledge:3)
- 절대 금지 카테고리: meme / 공감あるある / ぼやき (트위터 영역)
- 포맷 100% 카루셀 (S1 표지 / S2 훅 / S3 정보1 / S4 정보2 / S5 CTA)
- 주제 중복 금지 (과거 기획안과 비교)
- 톤: 「동네 언니」 정보+친근. 「必須/推奨/黄金ルール」 금지. 「〜してね/〜してあげて♡」 과도 친구톤 금지
- 오노마토페 카루셀 전체 0~1개
- 썸네일: subtitle(소제목 라벨) + title(대제목, 핵심 키워드 강조) 필수
- CTA 캐치프레이즈 후보 5개(1톤 2/3톤 1~2/4톤 1~2), 대표 1개 선정, JP+KO 병기
- 메인 액션 = 팔로우 OR 프로필 링크 (DM/쿠폰 단독 X)
- 경쟁사 출처 = 1차(경쟁사) + 보강(공식기관/논문/다른브랜드) 둘 다 명시
- 캡션 5~8행, 해시태그 「#グロミミ #grosmimi #ストローマグ #スマートマグ」 통일 포함""",
        "check_format": """검사 결과를 아래 JSON 형식으로 반환하세요:
{
  "pass": true/false,
  "score": 0-100,
  "violations": [{"rule": "규칙명", "item": "해당 기획", "fix": "수정 제안"}],
  "category_balance": {"tips": N, "brand": N, "k_babyfood": N, "knowledge": N},
  "duplicate_topics": ["중복된 주제 목록"],
  "tone_check": "톤 가이드 위반 여부 한 줄",
  "cta_check": "CTA 5후보 + 대표 1개 + 메인액션 룰 준수 여부 한 줄",
  "source_check": "1차+보강 출처 둘 다 채워졌는지 한 줄"
}"""
    },
