...
{{% comment %}}
  Login gate: show inline login prompt if not authenticated.
  Opens Shopify login in new tab; auto-reloads when user returns.
{{% endcomment %}}
{{% unless customer %}}
  <style>
    .igf-wrap,.igf-card,.igf-products-wrap{{display:none!important}}
    .igf-login-gate {{
      min-height: 70vh; display: flex; align-items: center; justify-content: center;
      padding: 32px 16px; background: #F5EEE6;
      font-family: var(--font-body-family, Fustat), sans-serif;
    }}
    .igf-login-card {{
      background: #fff; border-radius: 20px; padding: 48px 40px; max-width: 440px; width: 100%;
      box-shadow: 0 2px 16px rgba(45,43,40,0.05); border: 1px solid rgba(227,221,209,0.6);
      text-align: center;
    }}
    .igf-login-card h2 {{
      font-size: 1.5rem; color: #3D3929; margin: 0 0 8px; font-weight: 700;
    }}
    .igf-login-card p {{
      color: #9A917F; font-size: 0.95rem; line-height: 1.5; margin: 0 0 28px;
    }}
    .igf-login-btn {{
      display: inline-block; background: #D97757; color: #fff; border: none; border-radius: 12px;
      padding: 14px 36px; font-size: 1rem; font-weight: 600; cursor: pointer;
      text-decoration: none; transition: background 0.2s;
    }}
    .igf-login-btn:hover {{ background: #C16842; }}
    .igf-login-hint {{
      margin-top: 16px; font-size: 0.82rem; color: #bbb;
    }}
    @media (max-width: 480px) {{
      .igf-login-card {{ padding: 36px 24px; border-radius: 16px; }}
    }}
  </style>
  <div class="igf-login-gate">
  ...
  <script>
    // Auto-reload when user returns from login tab
