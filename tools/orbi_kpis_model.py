"""ORBI KPIs Model — upgraded from NoPolar Financial Model.

Summary-first architecture: Executive/Sales/Ads/UnitEcon summaries up front,
detailed analysis tabs in the back. Dual-audience: finance + non-finance readers.
"""
import json, os, re
...
# Output to Shared/ORBI KPIs/Output/
_SHARED_OUTPUT = os.path.normpath(os.path.join(
    ROOT, "..", "Shared", "ORBI KPIs", "Output"
))
OUT = get_output_path("orbi", "kpis_model", base_dir=_SHARED_OUTPUT)
...
def preprocess():
    ...
    # ── ORBI KPIs: monthly aggregations for summary tabs ──────────────────────
    # Monthly totals: {month -> {net, gross, disc, orders, ad_spend, ad_rev, cogs, fees}}
    monthly_totals = defaultdict(lambda: defaultdict(float))
    for (ch, brand), mo_data in rev_cb.items():
        if ch == "PR":
            continue
        for mo, metrics in mo_data.items():
            monthly_totals[mo]["net"] += metrics.get("net", 0)
            monthly_totals[mo]["gross"] += metrics.get("gross", 0)
            monthly_totals[mo]["disc"] += abs(metrics.get("disc", 0))
            monthly_totals[mo]["orders"] += metrics.get("orders", 0)

    # Ad spend/revenue totals per month
    for (plat, camp_name), mo_data in camp_monthly.items():
        for mo, metrics in mo_data.items():
            monthly_totals[mo]["ad_spend"] += metrics.get("spend", 0)
            monthly_totals[mo]["ad_rev"] += metrics.get("revenue", 0)
            monthly_totals[mo]["ad_clicks"] += metrics.get("clicks", 0)

    # COGS & Fees per month (from shopify + amz)
    for brand, mo_data in shopify.items():
        for mo, metrics in mo_data.items():
            monthly_totals[mo]["cogs"] += metrics.get("cogs", 0)
            monthly_totals[mo]["fees"] += metrics.get("txn_fees", 0)
    for brand, mo_data in amz.items():
        for mo, metrics in mo_data.items():
            monthly_totals[mo]["cogs"] += metrics.get("cogs", 0)
            monthly_totals[mo]["fees"] += metrics.get("fees", 0)

    # Channel monthly: {channel -> {month -> {net, orders, ad_spend, ad_rev}}}
    channel_monthly = defaultdict(lambda: defaultdict(lambda: defaultdict(float)))
    for (ch, brand), mo_data in rev_cb.items():
        if ch == "PR":
            continue
        for mo, metrics in mo_data.items():
            channel_monthly[ch][mo]["net"] += metrics.get("net", 0)
            channel_monthly[ch][mo]["gross"] += metrics.get("gross", 0)
            channel_monthly[ch][mo]["orders"] += metrics.get("orders", 0)
    for (land, brand), mo_data in ads_lb.items():
        for mo, metrics in mo_data.items():
            channel_monthly[land][mo]["ad_spend"] += metrics.get("spend", 0)
            channel_monthly[land][mo]["ad_rev"] += metrics.get("revenue", 0)

    # Brand monthly: {brand -> {month -> {net, orders, ad_spend}}}
    brand_monthly = defaultdict(lambda: defaultdict(lambda: defaultdict(float)))
    for (ch, brand), mo_data in rev_cb.items():
        if ch == "PR":
            continue
        for mo, metrics in mo_data.items():
            brand_monthly[brand][mo]["net"] += metrics.get("net", 0)
            brand_monthly[brand][mo]["gross"] += metrics.get("gross", 0)
            brand_monthly[brand][mo]["orders"] += metrics.get("orders", 0)
    for key, mo_data in ads_lb.items():
        brand = key[1]
        for mo, metrics in mo_data.items():
            brand_monthly[brand][mo]["ad_spend"] += metrics.get("spend", 0)
            brand_monthly[brand][mo]["ad_rev"] += metrics.get("revenue", 0)

    # Platform monthly: {platform -> {month -> {spend, revenue, clicks}}}
    platform_monthly = defaultdict(lambda: defaultdict(lambda: defaultdict(float)))
    for (plat, camp_name), mo_data in camp_monthly.items():
        for mo, metrics in mo_data.items():
            platform_monthly[plat][mo]["spend"] += metrics.get("spend", 0)
            platform_monthly[plat][mo]["revenue"] += metrics.get("revenue", 0)
            platform_monthly[plat][mo]["clicks"] += metrics.get("clicks", 0)
    ...
    for mo in months:
        ...
        gp = net - cogs
        cm_before = gp - fees
        cm_after = cm_before - ad_spend
        kpi_monthly[mo] = {
            "net": net, "gross": t["gross"], "disc": t["disc"],
            "orders": orders, "ad_spend": ad_spend, "ad_rev": ad_rev,
            "ad_clicks": t["ad_clicks"],
            "cogs": cogs, "fees": fees,
            "gp": gp, "cm_before": cm_before, "cm_after": cm_after,
            "aov": net / orders if orders else 0,
            "blended_cac": ad_spend / orders if orders else 0,
            "blended_roas": ad_rev / ad_spend if ad_spend else 0,
            "ad_spend_ratio": ad_spend / net if net else 0,
            "gross_margin": gp / net if net else 0,
            "cm_pct": cm_after / net if net else 0,
            "discount_rate": t["disc"] / t["gross"] if t["gross"] else 0,
            "cpc": ad_spend / t["ad_clicks"] if t["ad_clicks"] else 0,
        }

    print(f"  ORBI KPIs: {len(kpi_monthly)} months computed")
    ...
    return {
        ...
        "channel_monthly": {k: dict(v) for k, v in channel_monthly.items()},
        "brand_monthly": {k: dict(v) for k, v in brand_monthly.items()},
        "platform_monthly": {k: dict(v) for k, v in platform_monthly.items()},
...
# ══════════════════════════════════════════════════════════════════════════════
# ORBI KPIs SUMMARY TABS
# ══════════════════════════════════════════════════════════════════════════════

def _safe_div(a, b):
    return a / b if b else 0

def _mom_growth(cur, prev):
    if prev and prev != 0:
        return (cur - prev) / abs(prev)
    return None

def _yoy_month(m):
    """Return the same month one year prior, e.g. 2026-02 -> 2025-02."""
    y, mo = int(m[:4]), int(m[5:7])
    return f"{y-1}-{mo:02d}"


def _write_summary_header(ws, row, title, ncols, fill=None):
    """Write a dark header bar across ncols."""
    _fill = fill or DBLUE
...
def _write_summary_subheader(ws, row, title, ncols):
    for c in range(1, ncols + 1):
        cell = ws.cell(row=row, column=c)
        cell.fill = MBLUE
        cell.font = WF9
...
def _write_kpi_row(ws, row, label, values, fmt=DOL, label_col=1, data_start=2, bold=False):
    """Write a label + monthly values row."""
    cell = ws.cell(row=row, column=label_col, value=label)
    cell.font = BF if bold else NF
    cell.border = THIN
    for i, v in enumerate(values):
        cell = ws.cell(row=row, column=data_start + i)
        if v is not None:
            cell.value = v
        cell.number_format = fmt
        cell.font = BF if bold else NF
...
def build_executive_summary(wb, D):
    """Executive Summary: 1-page dashboard with key financial metrics.
    Designed for non-finance readers to grasp business health at a glance.
    """
    ...
    ncols = 1 + len(display_months) + 1  # label + months + YTD
    ytd_months = D["ytd_months"]
    row = 1

    # ── Month headers ──
    row = _write_summary_header(ws, row, "EXECUTIVE SUMMARY — ORBI KPIs", ncols)
    ws.cell(row=row, column=1, value="Metric").font = WF9
    ws.cell(row=row, column=1).fill = MBLUE
    ws.cell(row=row, column=1).border = THIN
    for i, m in enumerate(display_months):
        cell = ws.cell(row=row, column=2 + i, value=_month_label(m))
        cell.font = WF9
        cell.fill = MBLUE
        cell.border = THIN
        cell.alignment = Alignment(horizontal="center")
    ytd_cell = ws.cell(row=row, column=2 + len(display_months), value="YTD")
    ytd_cell.font = WF9
    ytd_cell.fill = MBLUE
    ytd_cell.border = THIN
    ytd_cell.alignment = Alignment(horizontal="center")
    row += 1

    def _ytd_sum(key):
        return sum(kpi.get(m, {}).get(key, 0) for m in ytd_months)

    def _ytd_ratio(num_key, den_key):
        n = sum(kpi.get(m, {}).get(num_key, 0) for m in ytd_months)
        d = sum(kpi.get(m, {}).get(den_key, 0) for m in ytd_months)
        return _safe_div(n, d)

    def _vals(key):
        return [kpi.get(m, {}).get(key, 0) for m in display_months]

    def _ratio_vals(num_key, den_key):
        result = []
        for m in display_months:
            n = kpi.get(m, {}).get(num_key, 0)
            d = kpi.get(m, {}).get(den_key, 0)
            result.append(_safe_div(n, d))
        return result

    # ── SECTION: Revenue ──
    row = _write_summary_subheader(ws, row, "REVENUE", ncols)
    row = _write_kpi_row(ws, row, "Net Revenue", _vals("net") + [_ytd_sum("net")], DOL, bold=True)
    row = _write_kpi_row(ws, row, "Gross Sales", _vals("gross") + [_ytd_sum("gross")], DOL)
    row = _write_kpi_row(ws, row, "Discounts", _vals("disc") + [_ytd_sum("disc")], DOL)
    row = _write_kpi_row(ws, row, "Discount Rate", _ratio_vals("disc", "gross") + [_ytd_ratio("disc", "gross")], PCT)
    row = _write_kpi_row(ws, row, "Total Orders", _vals("orders") + [_ytd_sum("orders")], NUM, bold=True)
    row = _write_kpi_row(ws, row, "AOV (Net/Orders)", _vals("aov") + [_ytd_ratio("net", "orders")], DOL2)
    ...
    for i, m in enumerate(display_months):
        if i == 0:
            ...
            prev_m = display_months[i - 1]
        cur = kpi.get(m, {}).get("net", 0)
        prev = kpi.get(prev_m, {}).get("net", 0) if prev_m else 0
        mom_vals.append(_mom_growth(cur, prev))
    row = _write_kpi_row(ws, row, "MoM Revenue Growth", mom_vals + [None], PCT)

    # YoY Growth
    yoy_vals = []
    for m in display_months:
        cur = kpi.get(m, {}).get("net", 0)
        prev = kpi.get(_yoy_month(m), {}).get("net", 0)
        yoy_vals.append(_mom_growth(cur, prev))
    row = _write_kpi_row(ws, row, "YoY Revenue Growth", yoy_vals + [None], PCT)
    row += 1

    # ── SECTION: Profitability ──
    row = _write_summary_subheader(ws, row, "PROFITABILITY", ncols)
    row = _write_kpi_row(ws, row, "COGS", _vals("cogs") + [_ytd_sum("cogs")], DOL)
    row = _write_kpi_row(ws, row, "Gross Profit", _vals("gp") + [_ytd_sum("gp")], DOL, bold=True)
    row = _write_kpi_row(ws, row, "Gross Margin %", _vals("gross_margin") + [_ytd_ratio("gp", "net")], PCT, bold=True)
    row = _write_kpi_row(ws, row, "Channel Fees", _vals("fees") + [_ytd_sum("fees")], DOL)
    row = _write_kpi_row(ws, row, "CM Before Ads", _vals("cm_before") + [_ytd_sum("cm_before")], DOL)
    row = _write_kpi_row(ws, row, "Ad Spend", _vals("ad_spend") + [_ytd_sum("ad_spend")], DOL)
    row = _write_kpi_row(ws, row, "CM After Ads", _vals("cm_after") + [_ytd_sum("cm_after")], DOL, bold=True)
    row = _write_kpi_row(ws, row, "CM %", _vals("cm_pct") + [_ytd_ratio("cm_after", "net")], PCT, bold=True)
    row += 1

    # ── SECTION: Advertising Efficiency ──
    row = _write_summary_subheader(ws, row, "ADVERTISING EFFICIENCY", ncols)
    row = _write_kpi_row(ws, row, "Total Ad Spend", _vals("ad_spend") + [_ytd_sum("ad_spend")], DOL)
    row = _write_kpi_row(ws, row, "Ad-Attributed Revenue", _vals("ad_rev") + [_ytd_sum("ad_rev")], DOL)
    row = _write_kpi_row(ws, row, "Blended ROAS", _vals("blended_roas") + [_ytd_ratio("ad_rev", "ad_spend")], DEC, bold=True)
    row = _write_kpi_row(ws, row, "Ad Spend / Revenue %", _vals("ad_spend_ratio") + [_ytd_ratio("ad_spend", "net")], PCT, bold=True)
    row = _write_kpi_row(ws, row, "Blended CAC", _vals("blended_cac") + [_ytd_ratio("ad_spend", "orders")], DOL2, bold=True)
    row = _write_kpi_row(ws, row, "Avg CPC", _vals("cpc") + [_ytd_ratio("ad_spend", "ad_clicks")], DOL2)
    row += 1

    # ── Column widths ──
    ws.column_dimensions["A"].width = 24
    for i in range(len(display_months) + 1):
        ws.column_dimensions[get_column_letter(2 + i)].width = 14
    ws.freeze_panes = "B3"
    print(f"  Executive Summary: {row} rows, {len(display_months)} months")


def build_sales_summary(wb, D):
    """Sales Summary: channel mix, brand mix, top products — trend-focused."""
    ws = wb.create_sheet("Sales Summary")
    ws.sheet_properties.tabColor = "4472C4"
    months = D["months"]
    kpi = D["kpi_monthly"]
    channel_monthly = D["channel_monthly"]
    brand_monthly = D["brand_monthly"]
    ytd_months = D["ytd_months"]

    display_months = months[-12:]
    ncols = 1 + len(display_months) + 1
    row = 1

    # ── Month headers ──
    row = _write_summary_header(ws, row, "SALES SUMMARY", ncols)
    ws.cell(row=row, column=1, value="").font = WF9
    ws.cell(row=row, column=1).fill = MBLUE
    ws.cell(row=row, column=1).border = THIN
    for i, m in enumerate(display_months):
        cell = ws.cell(row=row, column=2 + i, value=_month_label(m))
        ...
        cell.alignment = Alignment(horizontal="center")
    ytd_c = ws.cell(row=row, column=2 + len(display_months), value="YTD")
    ytd_c.font = WF9; ytd_c.fill = MBLUE; ytd_c.border = THIN
    ytd_c.alignment = Alignment(horizontal="center")
    row += 1

    # ── SECTION: Total Revenue ──
    row = _write_summary_subheader(ws, row, "TOTAL NET REVENUE", ncols)
    vals = [kpi.get(m, {}).get("net", 0) for m in display_months]
    ytd_total = sum(kpi.get(m, {}).get("net", 0) for m in ytd_months)
    row = _write_kpi_row(ws, row, "Total", vals + [ytd_total], DOL, bold=True)
    row += 1

    # ── SECTION: Revenue by Channel ──
    row = _write_summary_subheader(ws, row, "NET REVENUE BY CHANNEL", ncols)
    channels_sorted = sorted(
        channel_monthly.keys(),
        key=lambda ch: -sum(channel_monthly[ch].get(m, {}).get("net", 0) for m in ytd_months),
    )
    for ch in channels_sorted:
        ch_vals = [channel_monthly[ch].get(m, {}).get("net", 0) for m in display_months]
        ch_ytd = sum(channel_monthly[ch].get(m, {}).get("net", 0) for m in ytd_months)
        row = _write_kpi_row(ws, row, ch, ch_vals + [ch_ytd], DOL)
    row += 1

    # ── Channel Mix % ──
    row = _write_summary_subheader(ws, row, "CHANNEL MIX % (of Net Revenue)", ncols)
    for ch in channels_sorted:
        mix_vals = []
        for m in display_months:
            ch_net = channel_monthly[ch].get(m, {}).get("net", 0)
            total_net = kpi.get(m, {}).get("net", 0)
            mix_vals.append(_safe_div(ch_net, total_net))
        ch_ytd_net = sum(channel_monthly[ch].get(m, {}).get("net", 0) for m in ytd_months)
        mix_vals.append(_safe_div(ch_ytd_net, ytd_total))
        row = _write_kpi_row(ws, row, ch, mix_vals, PCT)
    row += 1

    # ── SECTION: Revenue by Brand ──
    row = _write_summary_subheader(ws, row, "NET REVENUE BY BRAND", ncols)
    brands_sorted = sorted(
        brand_monthly.keys(),
        key=lambda b: -sum(brand_monthly[b].get(m, {}).get("net", 0) for m in ytd_months),
    )
    for b in brands_sorted[:10]:  # Top 10
        b_vals = [brand_monthly[b].get(m, {}).get("net", 0) for m in display_months]
        b_ytd = sum(brand_monthly[b].get(m, {}).get("net", 0) for m in ytd_months)
        row = _write_kpi_row(ws, row, b, b_vals + [b_ytd], DOL)
    row += 1

    # ── Brand Mix % ──
    row = _write_summary_subheader(ws, row, "BRAND MIX % (of Net Revenue)", ncols)
    for b in brands_sorted[:10]:
        mix_vals = []
        for m in display_months:
            b_net = brand_monthly[b].get(m, {}).get("net", 0)
            total_net = kpi.get(m, {}).get("net", 0)
            mix_vals.append(_safe_div(b_net, total_net))
        b_ytd_net = sum(brand_monthly[b].get(m, {}).get("net", 0) for m in ytd_months)
        mix_vals.append(_safe_div(b_ytd_net, ytd_total))
        row = _write_kpi_row(ws, row, b, mix_vals, PCT)
    row += 1

    # ── Orders by Channel ──
    row = _write_summary_subheader(ws, row, "ORDERS BY CHANNEL", ncols)
    for ch in channels_sorted:
        o_vals = [channel_monthly[ch].get(m, {}).get("orders", 0) for m in display_months]
        o_ytd = sum(channel_monthly[ch].get(m, {}).get("orders", 0) for m in ytd_months)
        row = _write_kpi_row(ws, row, ch, o_vals + [o_ytd], NUM)
    row += 1

    # ── AOV by Channel ──
    row = _write_summary_subheader(ws, row, "AOV BY CHANNEL (Net / Orders)", ncols)
    for ch in channels_sorted:
        aov_vals = []
        for m in display_months:
            n = channel_monthly[ch].get(m, {}).get("net", 0)
            o = channel_monthly[ch].get(m, {}).get("orders", 0)
            aov_vals.append(_safe_div(n, o))
        ch_ytd_n = sum(channel_monthly[ch].get(m, {}).get("net", 0) for m in ytd_months)
        ch_ytd_o = sum(channel_monthly[ch].get(m, {}).get("orders", 0) for m in ytd_months)
        aov_vals.append(_safe_div(ch_ytd_n, ch_ytd_o))
        row = _write_kpi_row(ws, row, ch, aov_vals, DOL2)
    row += 1
    ...
    for i in range(len(display_months) + 1):
        ws.column_dimensions[get_column_letter(2 + i)].width = 14
    ws.freeze_panes = "B3"
    print(f"  Sales Summary: {row} rows")


def build_ads_summary(wb, D):
    """Ads Summary: platform efficiency, ROAS by platform/brand, campaign type mix."""
    ws = wb.create_sheet("Ads Summary")
    ws.sheet_properties.tabColor = "ED7D31"
    months = D["months"]
    kpi = D["kpi_monthly"]
    platform_monthly = D["platform_monthly"]
    brand_monthly = D["brand_monthly"]
    ytd_months = D["ytd_months"]
    ...
    # ── Month headers ──
    row = _write_summary_header(ws, row, "ADS SUMMARY", ncols)
    ws.cell(row=row, column=1, value="").font = WF9
    ws.cell(row=row, column=1).fill = MBLUE; ws.cell(row=row, column=1).border = THIN
    for i, m in enumerate(display_months):
        cell = ws.cell(row=row, column=2 + i, value=_month_label(m))
        cell.font = WF9; cell.fill = MBLUE; cell.border = THIN
        cell.alignment = Alignment(horizontal="center")
    ...
    # ── SECTION: Blended Totals ──
    row = _write_summary_subheader(ws, row, "BLENDED ADVERTISING METRICS", ncols)
    row = _write_kpi_row(ws, row, "Total Ad Spend", [kpi.get(m, {}).get("ad_spend", 0) for m in display_months] +
                          [sum(kpi.get(m, {}).get("ad_spend", 0) for m in ytd_months)], DOL, bold=True)
    row = _write_kpi_row(ws, row, "Ad-Attributed Revenue", [kpi.get(m, {}).get("ad_rev", 0) for m in display_months] +
                          [sum(kpi.get(m, {}).get("ad_rev", 0) for m in ytd_months)], DOL)
    row = _write_kpi_row(ws, row, "Blended ROAS", [kpi.get(m, {}).get("blended_roas", 0) for m in display_months] +
                          [_safe_div(sum(kpi.get(m, {}).get("ad_rev", 0) for m in ytd_months),
                                     sum(kpi.get(m, {}).get("ad_spend", 0) for m in ytd_months))], DEC, bold=True)
    row = _write_kpi_row(ws, row, "Ad Spend / Net Revenue", [kpi.get(m, {}).get("ad_spend_ratio", 0) for m in display_months] +
                          [_safe_div(sum(kpi.get(m, {}).get("ad_spend", 0) for m in ytd_months),
                                     sum(kpi.get(m, {}).get("net", 0) for m in ytd_months))], PCT, bold=True)
    row = _write_kpi_row(ws, row, "Blended CAC", [kpi.get(m, {}).get("blended_cac", 0) for m in display_months] +
                          [_safe_div(sum(kpi.get(m, {}).get("ad_spend", 0) for m in ytd_months),
                                     sum(kpi.get(m, {}).get("orders", 0) for m in ytd_months))], DOL2, bold=True)
    row = _write_kpi_row(ws, row, "Avg CPC", [kpi.get(m, {}).get("cpc", 0) for m in display_months] +
                          [_safe_div(sum(kpi.get(m, {}).get("ad_spend", 0) for m in ytd_months),
                                     sum(kpi.get(m, {}).get("ad_clicks", 0) for m in ytd_months))], DOL2)
    row += 1

    # ── SECTION: Spend by Platform ──
    row = _write_summary_subheader(ws, row, "AD SPEND BY PLATFORM", ncols)
    plats_sorted = sorted(
        platform_monthly.keys(),
        key=lambda p: -sum(platform_monthly[p].get(m, {}).get("spend", 0) for m in ytd_months),
    )
    for plat in plats_sorted:
        vals = [platform_monthly[plat].get(m, {}).get("spend", 0) for m in display_months]
        ytd_v = sum(platform_monthly[plat].get(m, {}).get("spend", 0) for m in ytd_months)
        row = _write_kpi_row(ws, row, plat, vals + [ytd_v], DOL)
    row += 1

    # ── ROAS by Platform ──
    row = _write_summary_subheader(ws, row, "ROAS BY PLATFORM", ncols)
    for plat in plats_sorted:
        roas_vals = []
        for m in display_months:
            s = platform_monthly[plat].get(m, {}).get("spend", 0)
            r = platform_monthly[plat].get(m, {}).get("revenue", 0)
            roas_vals.append(_safe_div(r, s))
        ytd_s = sum(platform_monthly[plat].get(m, {}).get("spend", 0) for m in ytd_months)
        ytd_r = sum(platform_monthly[plat].get(m, {}).get("revenue", 0) for m in ytd_months)
        roas_vals.append(_safe_div(ytd_r, ytd_s))
        row = _write_kpi_row(ws, row, plat, roas_vals, DEC, bold=True)
    row += 1

    # ── Platform Spend Mix % ──
    row = _write_summary_subheader(ws, row, "PLATFORM SPEND MIX %", ncols)
    for plat in plats_sorted:
        mix_vals = []
        for m in display_months:
            p_spend = platform_monthly[plat].get(m, {}).get("spend", 0)
            t_spend = kpi.get(m, {}).get("ad_spend", 0)
            mix_vals.append(_safe_div(p_spend, t_spend))
        ytd_ps = sum(platform_monthly[plat].get(m, {}).get("spend", 0) for m in ytd_months)
        ytd_ts = sum(kpi.get(m, {}).get("ad_spend", 0) for m in ytd_months)
        mix_vals.append(_safe_div(ytd_ps, ytd_ts))
        row = _write_kpi_row(ws, row, plat, mix_vals, PCT)
    row += 1

    # ── Ad Spend by Brand ──
    row = _write_summary_subheader(ws, row, "AD SPEND BY BRAND", ncols)
    brands_sorted = sorted(
        brand_monthly.keys(),
        key=lambda b: -sum(brand_monthly[b].get(m, {}).get("ad_spend", 0) for m in ytd_months),
    )
    for b in brands_sorted[:8]:
        b_vals = [brand_monthly[b].get(m, {}).get("ad_spend", 0) for m in display_months]
        b_ytd = sum(brand_monthly[b].get(m, {}).get("ad_spend", 0) for m in ytd_months)
        row = _write_kpi_row(ws, row, b, b_vals + [b_ytd], DOL)
    row += 1

    # ── ROAS by Brand ──
    row = _write_summary_subheader(ws, row, "ROAS BY BRAND", ncols)
    for b in brands_sorted[:8]:
        roas_vals = []
        for m in display_months:
            s = brand_monthly[b].get(m, {}).get("ad_spend", 0)
            r = brand_monthly[b].get(m, {}).get("ad_rev", 0)
            roas_vals.append(_safe_div(r, s))
        ytd_s = sum(brand_monthly[b].get(m, {}).get("ad_spend", 0) for m in ytd_months)
        ytd_r = sum(brand_monthly[b].get(m, {}).get("ad_rev", 0) for m in ytd_months)
        roas_vals.append(_safe_div(ytd_r, ytd_s))
        row = _write_kpi_row(ws, row, b, roas_vals, DEC, bold=True)
    row += 1

    # ── Top 5 Campaigns (by spend, last 30d from Summary data) ──
    camp_window = D.get("camp_window", {})
    if "30d" in camp_window:
        row = _write_summary_subheader(ws, row, "TOP 10 CAMPAIGNS BY SPEND (Last 30d)", ncols)
        top_camps = sorted(camp_window["30d"].items(), key=lambda x: -x[1]["spend"])[:10]
        headers = ["Platform", "Campaign", "Spend", "Revenue", "ROAS", "Type", "Brand"]
        for ci, h in enumerate(headers):
            cell = ws.cell(row=row, column=1 + ci, value=h)
            cell.font = HF; cell.fill = LGRAY; cell.border = THIN
        row += 1
        for (plat, camp), m in top_camps:
            ws.cell(row=row, column=1, value=plat).font = NF
            ws.cell(row=row, column=1).border = THIN
            ws.cell(row=row, column=2, value=camp[:50]).font = NF
            ws.cell(row=row, column=2).border = THIN
            c = ws.cell(row=row, column=3, value=m["spend"]); c.number_format = DOL; c.font = NF; c.border = THIN
            c = ws.cell(row=row, column=4, value=m["revenue"]); c.number_format = DOL; c.font = NF; c.border = THIN
            c = ws.cell(row=row, column=5, value=m["roas"]); c.number_format = DEC; c.font = BF; c.border = THIN
            ws.cell(row=row, column=6, value=m["type"]).font = NF
            ws.cell(row=row, column=6).border = THIN
            ws.cell(row=row, column=7, value=m["brand"]).font = NF
    ...
    for i in range(len(display_months) + 1):
        ws.column_dimensions[get_column_letter(2 + i)].width = 14
    ws.freeze_panes = "B3"
    print(f"  Ads Summary: {row} rows")


def build_unit_economics(wb, D):
    """Unit Economics: CAC, AOV, margin waterfall, channel-level efficiency."""
    ...
    # ── Month headers ──
    row = _write_summary_header(ws, row, "UNIT ECONOMICS", ncols)
    ws.cell(row=row, column=1, value="").font = WF9
    ws.cell(row=row, column=1).fill = MBLUE; ws.cell(row=row, column=1).border = THIN
    for i, m in enumerate(display_months):
        cell = ws.cell(row=row, column=2 + i, value=_month_label(m))
    ...
    # ── SECTION: Per-Order Waterfall ──
    row = _write_summary_subheader(ws, row, "PER-ORDER WATERFALL (Blended)", ncols)
    row = _write_kpi_row(ws, row, "AOV (Net Revenue / Order)",
                          [kpi.get(m, {}).get("aov", 0) for m in display_months] + [_ytd_ratio("net", "orders")], DOL2, bold=True)

    # COGS per order
    cpo_vals = [_safe_div(kpi.get(m, {}).get("cogs", 0), kpi.get(m, {}).get("orders", 0)) for m in display_months]
    cpo_ytd = _safe_div(_ytd_sum("cogs"), _ytd_sum("orders"))
    row = _write_kpi_row(ws, row, "(-) COGS per Order", cpo_vals + [cpo_ytd], DOL2)

    # GP per order
    gpo_vals = [_safe_div(kpi.get(m, {}).get("gp", 0), kpi.get(m, {}).get("orders", 0)) for m in display_months]
    gpo_ytd = _safe_div(_ytd_sum("gp"), _ytd_sum("orders"))
    row = _write_kpi_row(ws, row, "= Gross Profit per Order", gpo_vals + [gpo_ytd], DOL2, bold=True)

    # Fees per order
    fpo_vals = [_safe_div(kpi.get(m, {}).get("fees", 0), kpi.get(m, {}).get("orders", 0)) for m in display_months]
    fpo_ytd = _safe_div(_ytd_sum("fees"), _ytd_sum("orders"))
    row = _write_kpi_row(ws, row, "(-) Fees per Order", fpo_vals + [fpo_ytd], DOL2)

    # CAC (ad spend per order)
    row = _write_kpi_row(ws, row, "(-) CAC (Ad Spend / Order)",
                          [kpi.get(m, {}).get("blended_cac", 0) for m in display_months] + [_ytd_ratio("ad_spend", "orders")], DOL2, bold=True)

    # CM per order
    cmpo_vals = [_safe_div(kpi.get(m, {}).get("cm_after", 0), kpi.get(m, {}).get("orders", 0)) for m in display_months]
    cmpo_ytd = _safe_div(_ytd_sum("cm_after"), _ytd_sum("orders"))
    row = _write_kpi_row(ws, row, "= CM per Order (After Ads)", cmpo_vals + [cmpo_ytd], DOL2, bold=True)
    row += 1

    # ── SECTION: Margin Stack ──
    row = _write_summary_subheader(ws, row, "MARGIN STACK (% of Net Revenue)", ncols)
    row = _write_kpi_row(ws, row, "Gross Margin %",
                          [kpi.get(m, {}).get("gross_margin", 0) for m in display_months] + [_ytd_ratio("gp", "net")], PCT, bold=True)

    fee_pct = [_safe_div(kpi.get(m, {}).get("fees", 0), kpi.get(m, {}).get("net", 0)) for m in display_months]
    row = _write_kpi_row(ws, row, "(-) Fee %", fee_pct + [_ytd_ratio("fees", "net")], PCT)
    row = _write_kpi_row(ws, row, "(-) Ad Spend %",
                          [kpi.get(m, {}).get("ad_spend_ratio", 0) for m in display_months] + [_ytd_ratio("ad_spend", "net")], PCT, bold=True)
    row = _write_kpi_row(ws, row, "= CM % (After Ads)",
                          [kpi.get(m, {}).get("cm_pct", 0) for m in display_months] + [_ytd_ratio("cm_after", "net")], PCT, bold=True)
    row += 1

    # ── SECTION: CAC by Channel ──
    row = _write_summary_subheader(ws, row, "CAC BY CHANNEL (Channel Ad Spend / Channel Orders)", ncols)
    channels_sorted = sorted(
        channel_monthly.keys(),
        key=lambda ch: -sum(channel_monthly[ch].get(m, {}).get("net", 0) for m in ytd_months),
    )
    for ch in channels_sorted:
        cac_vals = []
        for m in display_months:
            s = channel_monthly[ch].get(m, {}).get("ad_spend", 0)
            o = channel_monthly[ch].get(m, {}).get("orders", 0)
            cac_vals.append(_safe_div(s, o))
        ytd_s = sum(channel_monthly[ch].get(m, {}).get("ad_spend", 0) for m in ytd_months)
        ytd_o = sum(channel_monthly[ch].get(m, {}).get("orders", 0) for m in ytd_months)
        cac_vals.append(_safe_div(ytd_s, ytd_o))
        row = _write_kpi_row(ws, row, ch, cac_vals, DOL2)
    row += 1

    # ── SECTION: Channel ROAS ──
    row = _write_summary_subheader(ws, row, "ROAS BY CHANNEL (Ad Revenue / Ad Spend)", ncols)
    for ch in channels_sorted:
        roas_vals = []
        for m in display_months:
            s = channel_monthly[ch].get(m, {}).get("ad_spend", 0)
            r = channel_monthly[ch].get(m, {}).get("ad_rev", 0)
            roas_vals.append(_safe_div(r, s))
        ytd_s = sum(channel_monthly[ch].get(m, {}).get("ad_spend", 0) for m in ytd_months)
        ytd_r = sum(channel_monthly[ch].get(m, {}).get("ad_rev", 0) for m in ytd_months)
        roas_vals.append(_safe_div(ytd_r, ytd_s))
        row = _write_kpi_row(ws, row, ch, roas_vals, DEC, bold=True)
    row += 1

    # ── SECTION: Efficiency Ratios by Brand ──
    row = _write_summary_subheader(ws, row, "AD SPEND / REVENUE % BY BRAND", ncols)
    brands_sorted = sorted(
        brand_monthly.keys(),
        key=lambda b: -sum(brand_monthly[b].get(m, {}).get("net", 0) for m in ytd_months),
    )
    for b in brands_sorted[:8]:
        ...
        for m in display_months:
            s = brand_monthly[b].get(m, {}).get("ad_spend", 0)
            n = brand_monthly[b].get(m, {}).get("net", 0)
            ratio_vals.append(_safe_div(s, n))
        ytd_s = sum(brand_monthly[b].get(m, {}).get("ad_spend", 0) for m in ytd_months)
        ytd_n = sum(brand_monthly[b].get(m, {}).get("net", 0) for m in ytd_months)
        ratio_vals.append(_safe_div(ytd_s, ytd_n))
        row = _write_kpi_row(ws, row, b, ratio_vals, PCT)
    row += 1
    ...
    for i in range(len(display_months) + 1):
        ws.column_dimensions[get_column_letter(2 + i)].width = 14
    ws.freeze_panes = "B3"
    print(f"  Unit Economics: {row} rows")
...
def main():
    ...
    print("Building ORBI KPIs model...")
    wb = Workbook()
    wb.remove(wb.active)

    # ════════════════════════════════════════════════════
    # SUMMARY ZONE (for everyone)
    # ════════════════════════════════════════════════════
    build_executive_summary(wb, D)    # Tab 1
    build_sales_summary(wb, D)        # Tab 2
    build_ads_summary(wb, D)          # Tab 3
    build_unit_economics(wb, D)       # Tab 4

    # ════════════════════════════════════════════════════
    # DETAIL ZONE (for analysts)
    # ════════════════════════════════════════════════════
    sec_map_1a, flat_1a = build_revenue(wb, D)   # Tab 5: Sales Detail
    build_ads(wb, D)                              # Tab 6: Ads Detail
    build_cm(wb, D)                               # Tab 7: CM
    build_organic(wb, D)                          # Tab 8: Organic Sales
    build_summary(wb, D)                          # Tab 9: Campaign Rankings
    build_vintage(wb, D)                          # Tab 10: ADS Campaign Details

    if D.get("promo_data") and PROMO_EVENTS:
        build_promo_analysis(wb, D)               # Tab 11: Promo Analysis

    if D.get("search_vol"):
        build_search_volume(wb, D)                # Tab 12: Search Volume

    if D.get("has_influencer_data"):
        build_influencer_dashboard(wb, D)         # Tab 13: Influencer
    ...
    build_model_check(wb, D)                      # Last: Model Check
    ...
    try:
        ...
        fallback = get_output_path("orbi", "kpis_model", base_dir=_SHARED_OUTPUT)
        wb.save(fallback)
