...
def build_proposal_html(proposals: List[Dict],
                        ...
                        keyword_data_status: str = "not_attempted",
...
def _build_keyword_html(kw_proposals: Optional[List[Dict]], label_start: int = 0, backtest: Optional[Dict] = None) -> str:
    ...
    # ---- Backtest Insights Section ----
    if backtest:
        bt_waste = backtest.get("waste_backtest", {})
        bt_bid = backtest.get("bid_backtest", {})
        bt_period = backtest.get("period", {})
        bt_timeline = backtest.get("timeline", [])
        top_terms = backtest.get("top_waste_terms", [])

        waste_save = bt_waste.get("total_simulated_save", 0)
        waste_pct = bt_waste.get("save_pct", 0)
        sim_roas = bt_bid.get("sim_roas", 0)
        actual_roas = bt_bid.get("actual_roas", 0)
        roas_delta = bt_bid.get("roas_delta", 0)

        # Top waste terms rows
        waste_rows = ""
        for t in top_terms[:10]:
            waste_rows += f"""<tr>
                <td style="padding:6px;border-bottom:1px solid #e9ecef;font-size:12px;">{t.get('search_term','')}</td>
                <td style="padding:6px;border-bottom:1px solid #e9ecef;text-align:right;">${t.get('actual_spend',0):,.2f}</td>
                <td style="padding:6px;border-bottom:1px solid #e9ecef;text-align:right;color:#28a745;font-weight:bold;">${t.get('would_save',0):,.2f}</td>
                <td style="padding:6px;border-bottom:1px solid #e9ecef;text-align:center;">{t.get('conversions',0)}</td>
            </tr>"""

        # Timeline sparkline (text-based)
        timeline_html = ""
        if bt_timeline:
            timeline_html = "<div style='margin-top:8px;font-size:11px;color:#666;'>"
            for t in bt_timeline[-3:]:
                bar_width = min(int(t["monthly_save"] / 50), 200)
                timeline_html += f"<div>{t['month']}: <span style='display:inline-block;background:#28a745;height:12px;width:{bar_width}px;border-radius:2px;vertical-align:middle;'></span> ${t['monthly_save']:,.0f} (cumul: ${t['cumulative']:,.0f})</div>"
            timeline_html += "</div>"

        sections.append(f"""
        <div style="background:#f0fdf4;border:1px solid #86efac;border-radius:8px;padding:16px;margin-top:16px;">
            <h3 style="margin:0 0 12px;color:#166534;">📊 Backtest Insights ({bt_period.get('days',30)}d: {bt_period.get('start','')} ~ {bt_period.get('end','')})</h3>
            <table style="width:100%;border-collapse:collapse;font-size:13px;">
                <tr>
                    <td style="padding:8px;"><b>Wasted Spend Savings</b></td>
                    <td style="padding:8px;text-align:right;font-size:18px;color:#166534;font-weight:bold;">${waste_save:,.0f} <span style="font-size:12px;">({waste_pct}%)</span></td>
                </tr>
                <tr>
                    <td style="padding:8px;"><b>Negated terms</b></td>
                    <td style="padding:8px;text-align:right;">{bt_waste.get('negated_terms_count',0)} search terms</td>
                </tr>
                <tr>
                    <td style="padding:8px;"><b>ROAS Improvement</b></td>
                    <td style="padding:8px;text-align:right;font-size:16px;">{actual_roas}x → <b style="color:#166534;">{sim_roas}x</b> <span style="font-size:12px;">(+{roas_delta}x)</span></td>
                </tr>
                <tr>
                    <td style="padding:8px;"><b>Under-performers / Scalable</b></td>
                    <td style="padding:8px;text-align:right;">{bt_bid.get('underperformer_count',0)} bid ↓ / {bt_bid.get('scalable_count',0)} bid ↑</td>
                </tr>
            </table>
            {timeline_html}
...
def send_proposal_email(proposals: List[Dict], to: str = DEFAULT_TO,
                        ...
                        sku_context: Optional[Dict] = None,
                        keyword_data_status: str = "not_attempted",
                        backtest: Optional[Dict] = None) -> Optional[str]:
