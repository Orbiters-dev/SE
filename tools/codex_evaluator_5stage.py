...
# Namespace tag biases retrieval toward past Codex (structural) audits,
# so numeric-only Gemini audits do not contaminate Codex context
# (prevents cross-evaluator false-positive reinforcement).

LIGHTRAG_URL = os.environ.get("LIGHTRAG_URL", "http://localhost:9621")
RAG_NAMESPACE_TAG = "codex-structural-policy"
...
def _rag_query(text: str, mode: str = "hybrid", top_k: int = 5) -> str:
    """Query LightRAG for related structural/policy context. Empty on failure."""
    try:
        import urllib.request
        # Bias retrieval toward Codex's own past audits + structural notes
        biased = f"[{RAG_NAMESPACE_TAG} / structural policy security functionality error-handling] {text}"
...
def _rag_index_result(result: dict, domain: str, ts: str):
    """Index Codex audit result into LightRAG (fire-and-forget).

    Namespaced header ensures Gemini retrieval won't preferentially surface
    Codex's verdicts (prevents evaluator self-reinforcement loop).
    """
    if not _rag_healthy():
        return
    try:
        ...
        if not content or len(content) < 50:
            return
        # Namespace tag in header biases future Codex-side retrieval
        doc = (f"# Codex Structural Audit [{domain.upper()}] [{RAG_NAMESPACE_TAG}] — {ts}\n\n"
               f"{content[:3000]}")
        ...
        print(f"[RAG] Indexed Codex audit result ({len(doc)} chars)", file=sys.stderr)
    except Exception as e:
        print(f"[RAG] Index failed (non-blocking): {e}", file=sys.stderr)

# ─── Evaluator system prompts ───────────────────────────────────────

AUDIT_SYSTEM = """You are a skeptical STRUCTURE / POLICY / SECURITY auditor for the ORBI system.
Your peer auditor (Gemini) handles NUMERIC and FACTUAL verification — do NOT repeat their work.

## YOUR SCOPE (what to evaluate)
1. FUNCTIONALITY (weight: CRITICAL) — Does control flow work? Any broken feature, unreachable code, missing branch = FAIL.
2. ERROR HANDLING & ROBUSTNESS (weight: HIGH) — Error propagation, exception swallowing, resource leaks, retries, timeouts.
3. CODE QUALITY (weight: MEDIUM) — Security (injection, auth, secrets), performance (N+1, unbounded loops), testability.
4. DESIGN & MAINTAINABILITY (weight: LOW) — Naming, modularity, separation of concerns.

## OUT OF YOUR SCOPE (Gemini handles these — do NOT flag)
- Arithmetic correctness, formula validation, sum/total checks
- Cross-reference between docstring claims and actual numbers
- Factual grounding of hardcoded values / dates / thresholds
- Percentage / ratio / unit consistency
- Numeric sanity (ACOS ranges, Gross Margin bounds, etc.)

If you find a numeric issue, note it as [GEMINI-TERRITORY] and move on — do NOT include it in your FAIL verdict.

## CALIBRATION RULES (avoid false positives)
- Intentional fallback behavior (returning `[]` on missing file) is a DESIGN CHOICE, not a bug, UNLESS the caller cannot distinguish success from failure. Flag only if the ambiguity is real.
- Sampling limits (e.g., "check top 20") are PERFORMANCE TRADE-OFFS, not bugs. Flag only if sampling provably hides known issue classes.
- SKIP-on-missing-file is often the CORRECT behavior when the caller explicitly expects optional inputs. Only flag if the docstring promises "fails on missing."
- If you are unsure whether something is a bug or design choice, mark as WARN, not FAIL.

## OUTPUT
For each file:
- Score per in-scope criterion (0-10)
- PASS/FAIL (FUNCTIONALITY < 8 = FAIL; others WARN unless critical)
- Specific issues with line numbers. Quote the code.
- Overall verdict: PASS | WARN | FAIL

Be specific. Quote code. Give line numbers. No vague praise. No numeric nitpicks."""
...
# "build" gate = auditing SOURCE CODE (is the code correctly written?)
# "test"  gate = auditing EXECUTION RESULTS (did the tests run properly? are results trustworthy?)

BUILD_GATE_SUFFIX = """

## GATE: BUILD (source code audit)
You are reading SOURCE CODE. Evaluate structural quality of the code itself.
Focus: control flow, error propagation, testability, security. Do NOT imagine what runtime output will look like — that's the TEST gate's job.
"""

TEST_GATE_SUFFIX = """

## GATE: TEST (execution-result audit)
You are reading EXECUTION RESULTS (JSON / HTML report / log output), not source code.
Evaluate whether the test run was valid on the STRUCTURAL axis:
- Did every declared check actually execute? Any silent SKIP = FAIL.
- Is coverage complete across declared dimensions (brands, channels, stages, etc.)?
- Are there signs of false negatives (tests passed but shouldn't have)?
- Are error handlers actually exercised, or did all paths hit happy-path?
Ignore "how pretty is the test code" — you're auditing the RUN, not the source.
"""
...
def _get_domain_prompt(args) -> str:
    """Return the system prompt for the selected domain + gate."""
    ...
    if gate == "test":
        ...
        # build is default — explicit suffix keeps LLM context consistent
        body = base + BUILD_GATE_SUFFIX
    # ─── Anti-stale-knowledge guard ──────────────────────────────────
    # Inject today's date so models don't treat valid recent dates as "future".
