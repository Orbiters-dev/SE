...
EXCEL_PATH = Path(__file__).parent.parent / "Japan_Marketing Plan_Monthly_V7.xlsx"

# ── KRW → JPY conversion ─────────────────────────────────────────
# Meta ad account currency is KRW; dashboard needs JPY.
KRW_TO_JPY = None

def get_krw_to_jpy():
    """Fetch live KRW→JPY rate, cache for session."""
    global KRW_TO_JPY
...
def update_meta(wb, year, month):
    ...
    # Get exchange rate (Meta account currency = KRW, dashboard = JPY)
    fx = get_krw_to_jpy()
    ...
    for ci, camp in enumerate(campaigns[:5]):
        ...
        spend_krw = float(camp.get("spend", 0))
        spend_jpy = round(spend_krw * fx)  # Convert KRW → JPY
        ...
        ws.cell(row=r, column=5).value = spend_jpy # E: Ad Spend (JPY)
        ...
        print(f"    [{ci+1}] {name}: ₩{spend_krw:,.0f} → ¥{spend_jpy:,} | {impr:,} impr | {clicks:,} clicks")
...
def update(year=None, month=None, week=None, search_volume=False, all_months=False):
    ...
    print("=" * 60)
    print(f"V7 Dashboard Update \u2014 {year}-{month:02d}")
    if all_months:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Update V7 dashboard with API data")
