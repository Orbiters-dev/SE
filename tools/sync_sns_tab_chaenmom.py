...
Usage:
  python tools/sync_sns_tab_chaenmom.py
  python tools/sync_sns_tab_chaenmom.py --dry-run
  python tools/sync_sns_tab_chaenmom.py --since 2026-01-01
"""
...
DEFAULT_TARGET_SHEET_ID = "16XUPd-VMoh6LEjSvupsmhkd1vEQNOTcoEhRCnPqkA_I"
SNS_TAB = "SNS"
...
def has_brand_product(order):
    """Check if order contains at least one CHA&MOM product."""
    for item in order.get("line_items", []):
        if BRAND_KEYWORD in item.get("title", "").lower():
            return True
    return False
...
def classify_product_type(order):
    """Get CHA&MOM product names from line items (non-CHA&MOM filtered out)."""
    items = order.get("line_items", [])
    names = []
    for item in items:
        title = item.get("title", "")
        if title and BRAND_KEYWORD in title.lower():
            names.append(title)
...
def build_rows(orders, paypal_txns, syncly_data, since_date=None):
    """Build SNS tab rows (CHA&MOM only)."""
    paid_people, person_fees = build_paid_set(orders, paypal_txns)
    syncly_idx = build_syncly_index(syncly_data)

    # Filter orders: shipped only, CHA&MOM only, no giveaway/events, since date
    filtered = []
    for order in orders:
        fs = order.get("fulfillment_status") or ""
        if fs not in ("fulfilled", "shipped"):
            continue
        if not has_brand_product(order):
            continue
...
def main():
    parser = argparse.ArgumentParser(
        description="Sync SNS tab (CHA&MOM): Shopify orders + PayPal + Syncly metrics"
    )
    parser.add_argument(
