"""Dongkyun Tester (CHA&MOM) - SNS Tab Sync pipeline validator for CHA&MOM brand.
...
Usage:
  python tools/dongkyun_tester_chaenmom.py --run            # full validation
  python tools/dongkyun_tester_chaenmom.py --validate-only   # skip API, check local data
  python tools/dongkyun_tester_chaenmom.py --results          # show last results
"""
...
RESULTS_PATH = PROJECT_ROOT / ".tmp" / "dongkyun_chaenmom_test_results.json"
...
TARGET_SHEET_ID = "16XUPd-VMoh6LEjSvupsmhkd1vEQNOTcoEhRCnPqkA_I"

BRAND_KEYWORD = "cha&mom"
...
def check_data_sources():
    ...
    if Q10_PATH.exists():
        try:
            ...
            # Count shipped + CHA&MOM
            shipped_brand = sum(
                1 for o in orders
                if (o.get("fulfillment_status") or "") in ("fulfilled", "shipped")
                and any(BRAND_KEYWORD in li.get("title", "").lower()
                        for li in o.get("line_items", []))
            )
            results.append(_pass("D3", f"Shipped CHA&MOM orders: {shipped_brand}"))
        except Exception as e:
...
def check_target(gc):
    ...
    try:
        ...
        try:
            ...
            results.append(_warn("T1", "SNS tab not yet created (will be created on first sync)"))
...
def check_matching():
    ...
    total = len(shipped_brand)

    if total == 0:
        results.append(_warn("M1", "No shipped CHA&MOM orders found"))
        return results

    ig_count = 0
    tt_count = 0
    unknown_count = 0
    for o in shipped_brand:
        tags = o.get("tags", "")
        note = o.get("note", "") or ""
        text = f"{tags} {note}"
...
def check_filtering():
    ...
    brand_count = sum(
        1 for o in shipped
        if any(BRAND_KEYWORD in li.get("title", "").lower()
               for li in o.get("line_items", []))
    )
    non_brand = len(shipped) - brand_count
    results.append(_pass("F1",
        f"CHA&MOM filter: {brand_count} pass / {non_brand} filtered out of {len(shipped)} shipped"))
...
def check_output():
    """Run sync_sns_tab_chaenmom.py build_rows and validate output."""
    results = []
    try:
        from sync_sns_tab_chaenmom import (
        ...
        if total > 0:
            ...
            results.append(_warn("O2", "No CHA&MOM rows generated (may have few orders)"))
...
def run_all(skip_api=False):
    ...
    print(f"\n=== Dongkyun Tester (CHA&MOM) === {timestamp}\n")
...
def main():
    parser = argparse.ArgumentParser(description="Dongkyun Tester (CHA&MOM): SNS Tab Sync validator")
