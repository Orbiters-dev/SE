...
# ─── Codex Auditor (lazy import for harness mode) ──────────────────────────
codex_auditor = None
def _load_codex_auditor():
    global codex_auditor
    if codex_auditor is None:
        import codex_auditor as _ca
        codex_auditor = _ca
    return codex_auditor
...
def generate_merged_report(config, executor_log, verifier_log, codex_verdicts=None):
...
"""
    ...
    # ─── Codex Flow Auditor Verdicts ────────────────────────────────────────
    if codex_verdicts:
        html += """
<div class="stage-card" style="border-left: 4px solid #f59e0b;">
  <div class="stage-header" style="background: linear-gradient(135deg, #fef3c7, #fff7ed);">
    <span class="stage-title">Codex Flow Auditor — Independent Evaluation</span>
  </div>
  <div style="padding: 16px 20px;">
"""
        for v in codex_verdicts:
            v_verdict = v.get("verdict", "ERROR")
            v_round = v.get("round", "?")
            v_passed = v.get("checks_passed", 0)
            v_total = v.get("checks_total", 0)
            v_badge = {"PASS": "badge-pass", "DEGRADED": "badge-warn", "FAIL": "badge-fail"}.get(v_verdict, "badge-fail")

            html += f"""
    <div style="margin-bottom: 14px; padding: 12px; background: #fafbfc; border-radius: 8px; border: 1px solid #e5e7eb;">
      <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px;">
        <strong>Round {v_round}</strong>
        <span class="badge {v_badge}">{v_verdict}</span>
      </div>
      <div style="font-size: 13px; color: #6b7280;">Checks: {v_passed}/{v_total}</div>
"""
            ...
            if failures:
                ...
                for f in failures:
                    sev = f.get("severity", "?")
                    sev_color = {"CRITICAL": "#dc2626", "HIGH": "#ea580c", "MEDIUM": "#d97706", "LOW": "#6b7280"}.get(sev, "#6b7280")
                    html += f'        <div class="check check-fail"><span class="check-icon">X</span> <span style="color:{sev_color};font-weight:600;">[{sev}]</span> {f.get("check","?")} — expected: {f.get("expected","?")}, actual: {f.get("actual","?")}</div>\n'
                html += '      </div>\n'
            ...
            if warnings:
                ...
                for w in warnings:
                    html += f'        <div class="check check-skip"><span class="check-icon">~</span> {w}</div>\n'
                html += '      </div>\n'

            notes = v.get("notes", "")
            if notes:
                html += f'      <div style="margin-top: 6px; font-size: 12px; color: #9ca3af; font-style: italic;">{notes}</div>\n'

            html += '    </div>\n'

        # Final harness verdict
        final_v = codex_verdicts[-1].get("verdict", "ERROR")
        rounds_used = len(codex_verdicts)
        final_badge = {"PASS": "badge-pass", "DEGRADED": "badge-warn", "FAIL": "badge-fail"}.get(final_v, "badge-fail")
        html += f"""
    <div style="text-align: center; padding: 12px; margin-top: 8px;">
      <span style="font-size: 18px; font-weight: 700;">Final Harness Verdict</span>
      <span class="badge {final_badge}" style="font-size: 16px; padding: 6px 20px;">{final_v}</span>
      <div style="font-size: 12px; color: #9ca3af; margin-top: 4px;">Completed in {rounds_used} round(s)</div>
    </div>
"""
        html += '  </div>\n</div>\n'
    ...
    html += f"""
<div class="footer">
  Generated by dual_test_runner.py | {now_str} | {'Harness Mode (Builder + Codex Evaluator)' if codex_verdicts else 'Maker-Checker Pattern'}
</div>
</body>
</html>"""
...
def run_dual(config, dry_run=False, no_cleanup=False, skip_report=False):
    """Core dual test logic — returns (exec_log, veri_log, report_path).
    Extracted so cmd_harness() can call it per round."""
    ...
    return exec_log, veri_log, report_path
...
def cmd_dual(stages, email=None, dry_run=False, no_cleanup=False, label=None):
    ...
    exec_log, veri_log, report_path = run_dual(config, dry_run=dry_run, no_cleanup=no_cleanup)
...
# ═══════════════════════════════════════════════════════════════════════════
# HARNESS MODE — 2-Round Codex Loop (Builder → Evaluator → loop)
# ══════════════��═══════════════════════════════════���════════════════════════

def cmd_harness(stages, email=None, dry_run=False, no_cleanup=False, label=None, max_rounds=2):
    """2-Round harness: Pipeliner (Builder) + Codex Flow Auditor (Evaluator).

    Round 1: Run full dual test → Codex audits → PASS? done : Round 2
    Round 2: Re-run FAIL stages → Codex re-audits → Final verdict
    """
    ca = _load_codex_auditor()

    config = DualTestConfig(stages=stages, email=email, label=label or "harness")
    config.save()
    all_stages = config.stages[:]

    log(f"\n{'*'*70}")
    log(f"  HARNESS MODE: Pipeliner (Builder) + Codex (Evaluator)")
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        # Step 1: Builder — run dual test
        if round_num > 1:
            # Round 2+: only re-run stages that had failures
            prev_verdict = all_verdicts[-1]
            failed_stage_names = [f["check"].split(":")[0] for f in prev_verdict.get("failures", [])
                                  if f.get("severity") in ("CRITICAL", "HIGH")]
            # Also get failed stages from verifier log
            failed_from_log = ca.extract_failed_stages(final_veri_log)
            retry_stages = list(dict.fromkeys(failed_stage_names + failed_from_log))  # dedup, preserve order

            # Filter to only stages that exist in the original list
            retry_stages = [s for s in retry_stages if s in all_stages]
            if not retry_stages:
                # If no specific failures found, re-run all stages
                retry_stages = all_stages[:]
            config.stages = retry_stages
            log(f"  Re-running failed stages: {' -> '.join(retry_stages)}")

        exec_log, veri_log, _ = run_dual(config, dry_run=dry_run,
                                          no_cleanup=True,  # keep data between rounds
                                          skip_report=True)  # report at end
        final_exec_log = exec_log
        final_veri_log = veri_log

        # Step 2: Evaluator — Codex audits
        prev = all_verdicts[-1] if all_verdicts else None
        if dry_run:
            log("  [DRY-RUN] Skipping Codex audit")
            verdict = {
                "verdict": "PASS", "domain": "pipeline", "round": round_num,
                "run_id": config.run_id, "checks_passed": 0, "checks_total": 0,
                "failures": [], "warnings": ["dry-run mode"], "notes": "Dry run — no Codex call",
            }
        else:
            verdict = ca.run_audit(config.run_id, round_num, prev_verdict=prev)
        all_verdicts.append(verdict)

        # Step 3: Decision
        v = verdict.get("verdict", "ERROR")
        log(f"\n  Round {round_num} verdict: {v}")

        if v in ("PASS", "DEGRADED"):
            log(f"  Pipeline PASSED (Round {round_num})")
            break
        elif round_num == max_rounds:
            log(f"  Pipeline FAILED after {max_rounds} rounds")

    # Final report with Codex verdicts
    report_path = generate_merged_report(config, final_exec_log, final_veri_log,
                                          codex_verdicts=all_verdicts)

    # Cleanup
    if not dry_run and not no_cleanup:
        signal_data = SignalFile.read(config.signal_path) if os.path.isfile(config.signal_path) else {
            "status": "finished", "stages_completed": []
        }
        run_cleanup(config, signal_data)

    # Summary
    final_verdict = all_verdicts[-1].get("verdict", "ERROR") if all_verdicts else "ERROR"
    rounds_used = len(all_verdicts)

    log(f"\n{'='*70}")
    log(f"  HARNESS COMPLETE — {final_verdict}")
    log(f"  Rounds: {rounds_used}/{max_rounds}")
    log(f"  Report: {report_path}")
    for i, v in enumerate(all_verdicts, 1):
        log(f"  Round {i}: {v.get('verdict')} ({v.get('checks_passed','?')}/{v.get('checks_total','?')})")
    log(f"{'='*70}\n")
...
def main():
    ...
    parser.add_argument("--harness", action="store_true", help="Harness mode: 2-round Codex loop (Builder + Evaluator)")
    parser.add_argument("--max-rounds", type=int, default=2, help="Max rounds for harness mode (default: 2)")
    ...
    if args.harness:
        report = cmd_harness(stages, email=args.email, dry_run=args.dry_run,
