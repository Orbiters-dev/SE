...
def execute_fixes(static_issues: list[Issue]) -> list[str]:
    ...
    - Creates/updates workflows/_orphaned_tools.md listing all orphan tools

    Prints full diff to stdout before applying.
    Writes scratch copy to .tmp/workflow_analyzer_preview/ (disposable).
    ...
    # Fix 2: Create/update workflows/_orphaned_tools.md
...
def _apply_fix(
    ...
    """Print diff to stdout, write scratch copy to .tmp/, apply to actual file."""
    import difflib
    ...
    # Scratch copy in .tmp/ (disposable per CLAUDE.md)
...
def build_html(
    ...
    gh_link = f"https://github.com/{GH_REPO}" if GH_REPO else "#"
