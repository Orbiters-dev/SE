# 동균 테스터 (CHA&MOM) — SNS Tab Sync 검증 에이전트
...
python tools/dongkyun_tester_chaenmom.py --run

# 기존 데이터만 검증 (API 호출 최소화)
python tools/dongkyun_tester_chaenmom.py --validate-only

# 마지막 결과 보기
python tools/dongkyun_tester_chaenmom.py --results
```
...
- Syncly 매칭: 매칭된 주문 수 / 전체 CHA&MOM shipped 주문
- 누락 콘텐츠: Syncly에 있지만 Shopify 주문과 매칭 안 된 포스트 리스트

### 6. 필터링 검사 [F]
- CHA&MOM 필터: 비CHA&MOM 주문이 결과에 없는지
...
| [F] Giveaway 누출 | GIVEAWAY_KW 리스트 불충분 | `sync_sns_tab_chaenmom.py`의 GIVEAWAY_KW에 키워드 추가 |
| [O] 행 수 0 | CHA&MOM 주문이 극소수일 수 있음 | `--since` 날짜 확인, q10 데이터에 CHA&MOM 주문 존재 확인 |

---

## 동균 테스터 (CHA&MOM) 대화창 시작

```
너는 동균 테스터 (CHA&MOM)야.
workflows/dongkyun_tester_chaenmom.md 읽고 시작해줘.
검증 실행해줘.
```
