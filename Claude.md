...
세션 시작 시 반드시:
1. 최근 60시간 git log 확인: `git log --since="60 hours ago" --oneline --all`
2. `memory/` 폴더에서 가장 최근 `session_*.md` 파일을 읽어 이전 세션 맥락 파악
3. 미완료 작업이 있으면 세은에게 먼저 알림

# Session End (자동)

세션이 끝나기 전 (compact 전, 또는 세은이 대화를 마무리할 때) 반드시:
1. `memory/session_YYYYMMDD.md` 에 오늘 세션 요약 저장 (진행한 작업, 미완료 작업, 피드백)
2. 인플루언서 메모리 파일 중 상태 변경된 건 업데이트
3. MEMORY.md 세션 요약 섹션에 링크 추가/갱신

**세은이 "세션 정리해" 안 해도 알아서 저장한다. 절대 빼먹지 마.**
...
## Harness (Builder ↔ Auditor 2-Loop)

모든 산출물에 OpenAI Codex Auditor를 붙여서 2회 루프 검사 후 보고한다.

### 자동 적용 대상

| 업무 | task_type | 적용 시점 |
|------|-----------|----------|
| 인플루언서 DM | `dm` | DM 초안 작성 후 세은에게 보여주기 전 |
| Python 코드 작성 | `code` | 새 스크립트/큰 수정 후 |
| n8n 워크플로우 | `workflow` | 워크플로우 생성/수정 시 |
| IG 기획안 | `ig_plan` | 주간 기획안 생성 후 |
| KPI/리포트 | `report` | 리포트 데이터 생성 후 |
| PPC 실행 | `ppc` | 입찰/예산 변경 제안 시 |

### 동작 방식

```
Builder (Claude) → 초안 생성
    ↓
Auditor (OpenAI GPT-4o) → 검사 Loop 1
    ↓ (violations 있으면)
Builder → 자동 수정
    ↓
Auditor → 재검사 Loop 2
    ↓
세은에게 최종 보고 (PASS/FAIL + Score + 위반사항)
```
...
- **DM은 반드시** Harness 통과 후 세은에게 보여준다
- PASS(Score 80+)면 바로 제시, FAIL이면 위반사항 + 수정본 함께 제시
- 검사 결과는 `.tmp/harness_results/`에 자동 저장
- 검사 로그는 `.tmp/auditor_logs/`에 자동 저장
- `PYTHONIOENCODING=utf-8` 필수 (cp949 방지)
...
## ツイッター部隊 (Twitter Squad — Grosmimi Japan)

"ツイッター部隊", "트위터 부대", "Twitter 부대", "총감독", "감독", "기획맨", "코멘터", "조사맨", "해시태그 조사" 명령이 오면 즉시 아래를 실행한다:

나는 **ツイッター部隊**를 통합 지휘하는 에이전트다.
@grosmimi_jp 계정의 트위터 운영 전체(기획→승인→투고→댓글→조사)를 자동화한다.

### 부대 구성·서열

```
総監督 (soukantoku.py)
  └─ 監督 (twitter_supervisor.py)
       ├─ 企画マン (twitter_scheduler.py --generate-weekly)
       ├─ ツイート (twitter_scheduler.py --slot 10/19)
       ├─ コメンター (twitter_engage.py --limit 1)
       ├─ 調査マン (twitter_research.py)
       └─ ハッシュタグ調査マン (twitter_hashtag.py)
```

| 이름 | 역할 | 실행 시간 (JST) | 도구 | GitHub Actions |
|------|------|----------------|------|---------------|
| **総監督** | 어제 전 부대 실적 Teams 보고 | 매일 09:00 | `tools/soukantoku.py` | `soukantoku.yml` |
| **監督** | 오늘 투고 예정·승인 상태 Teams 보고 | 매일 09:00 | `tools/twitter_supervisor.py` | `kantoku.yml` |
| **企画マン** | 다음주 7일분 트윗 플랜 생성 → Excel → Teams | 매주 금 09:00 | `twitter_scheduler.py --generate-weekly` | `kikaku.yml` |
| **ツイート** | 승인된 트윗 자동 투고 | 매일 10:00 / 19:00 | `twitter_scheduler.py --slot` | `tweet.yml` |
| **コメンター** | 육아 트윗에 공감 댓글 (1회 1명) | 매일 12~16시 (5회) | `tools/twitter_engage.py --limit 1` | `commenter.yml` |
| **調査マン** | 경쟁 브랜드 트위터 운영 조사 → Teams | 매주 수·금 09:00 | `tools/twitter_research.py` | `chousa.yml` |
| **ハッシュタグ調査マン** | 인기 해시태그 전체·육아 조사 → Teams | 매주 금 09:00 | `tools/twitter_hashtag.py` | `hashtag.yml` |

### 주간 플로우

```
금요일 09:00 — 企画マン: 다음주 7일분 플랜 생성 + Excel + Teams
금~일 — 담당자: Excel F열 승인 (Confirmed/Declined+대안)
월~일 매일:
  09:00 総監督 + 監督 보고
  10:00 ツイート 아침 투고
  12~16시 コメンター 공감 댓글 5회
  19:00 ツイート 저녁 투고
  수·금 09:00 調査マン 경쟁 조사
  금 09:00 ハッシュタグ調査マン
```

### 콘텐츠 카테고리 (企画マン 자동 로테이션)

| 카테고리 | 내용 | 슬롯 |
|----------|------|------|
| `meme` | 이야이야기 あるある, 육아 유머 | 10:00 |
| `tips` | 유아식·컵 마시기·재우기 실용 팁 | 10:00 |
| `brand` | 그로미미 제품 1세 10개월 사용 장면 | 19:00 |
| `k_babyfood` | 한국식 유아식 트렌드 소개 | 19:00 |
| `lifestyle` | 1세 10개월과의 일상·성장 기록 | 19:00 |

### 승인 룰 (Excel F열)

| F열 | G열 (대안) | 동작 |
|-----|-----------|------|
| Confirmed | 불필요 | 플랜 그대로 투고 |
| Declined | 텍스트 있음 | G열 텍스트로 투고 |
| Declined | 빈칸 | 스킵 |
| 빈칸 | — | 투고 안 함 (승인 대기) |

### コメンター 필터 룰

- **BLOCKED_ACCOUNTS**: pigeon, combi, richell, nuk, boon, munchkin, thermos 등 경쟁·자사
- **BLOCKED_KEYWORDS**: 漫画, アニメ, 声優, コスプレ, VTuber, にじさんじ, ホロライブ 등
- 키워드 필터 → Claude 판정 순서. 남성·기업 계정 스킵

### 추가 도구

| 도구 | 역할 |
|------|------|
| `tools/apify_twitter_monitor.py` | Apify로 트위터 모니터링 |
| `tools/apify_twitter_trends.py` | Apify로 트위터 트렌드 수집 |
| `tools/twitter_agent.py` | 트위터 에이전트 코어 |
| `tools/twitter_analytics.py` | 트위터 분석 |
| `tools/twitter_post.py` | 트윗 투고 |
| `tools/twitter_reply.py` | 트윗 답글 |
| `tools/twitter_trends.py` | 트렌드 수집 |
| `tools/twitter_utils.py` | 유틸리티 |

### 워크플로우

- `workflows/twitter_daily_posting.md` — ツイッター部隊 전체 운영 SOP

### 트리거 키워드

ツイッター部隊, 트위터 부대, Twitter 부대, 총감독, 감독, 기획맨, 코멘터, 조사맨, 해시태그 조사, twitter squad, soukantoku, kantoku, kikaku, commenter, chousa

---

## kazuki 추가 도구 (Rakuten·Amazon·기타)

kazuki 폴더에서 가져온 추가 에이전트/도구:

### Rakuten 운영

| 도구 | 역할 |
|------|------|
| `tools/rakuten_daily_report.py` | 라쿠텐 일일 리포트 |
| `tools/rakuten_review_workflow.py` | 라쿠텐 리뷰 워크플로우 |
| `tools/rakuten_rms_client.py` | RMS API 클라이언트 |
| `tools/rms_review_playwright.py` | RMS 리뷰 Playwright 크롤링 |
| `tools/rakuten_order_report.py` | 라쿠텐 주문 리포트 |
| `tools/send_rakuten_review_emails.py` | 라쿠텐 리뷰 이메일 발송 |
| `tools/send_approved_from_sheet.py` | 시트에서 승인된 항목 발송 |

### Amazon 운영

| 도구 | 역할 |
|------|------|
| `tools/amazon_daily_report.py` | 아마존 일일 리포트 |

### 기타

| 도구 | 역할 |
|------|------|
| `tools/selector_notify.py` | 선택자 알림 |
| `tools/track_delivery.py` | 배송 추적 |
| `tools/create_exhibition_schedule_notion.py` | 전시 스케줄 Notion 생성 |
| `tools/export_labels.py` | 라벨 내보내기 |
| `tools/generate_rakuten_banner.py` | 라쿠텐 배너 생성 |
| `tools/classify_polar_ads.py` | Polar 광고 분류 |
| `tools/polar_dashboard_builder.py` | Polar 대시보드 빌더 |
| `tools/polar_financial_model.py` | Polar 재무 모델 |

### 추가 워크플로우

| 워크플로우 | 내용 |
|-----------|------|
| `workflows/grosmimi_monitor_recruitment.md` | 그로미미 모니터 채용 |
| `workflows/polar_dashboard_builder.md` | Polar 대시보드 SOP |
| `workflows/polar_financial_model.md` | Polar 재무 모델 SOP |
